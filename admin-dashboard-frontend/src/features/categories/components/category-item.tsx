...
export function CategoryItem({ category, staffCount, onEdit, onDeleteRequest }: CategoryItemProps) {
  ...
  const handleAddSub = () => {
    ...
    updateMut.mutate(
      { id: category.id, data: { sub: [...subCategories, value] } },
      { onSuccess: () => setSubDraft("") }
    );
  };

  const handleRemoveSub = (sub: string) => {
    updateMut.mutate({ id: category.id, data: { sub: subCategories.filter((s) => s !== sub) } });
  };

  return (
    <Card className="rounded-2xl border-slate-200/80 shadow-md shadow-slate-100/50 bg-white overflow-hidden transition-all duration-300 hover:border-blue-300 hover:shadow-lg hover:shadow-slate-200/60 hover:-translate-y-0.5">
      <CardHeader className="flex-row items-center gap-4 bg-gradient-to-r from-slate-50 to-blue-50/40 border-b border-slate-100 pb-4 pt-5 px-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs font-semibold text-slate-600 mt-1 uppercase tracking-wider">
            {staffCount} qualified · CHF {category.defaultRate}/h · {subCategories.length} sub
          </p>
        </div>
      ...
      <CardContent className="space-y-4 p-5">
          ...
          {subCategories.length ? (
            subCategories.map((s) => (
              <Badge
                key={s}
                variant="secondary"
                className="pl-3 pr-1.5 py-1 gap-1.5 bg-blue-50 hover:bg-blue-100 text-blue-800 border border-blue-200 rounded-lg text-xs font-semibold transition-all"
              >
                {s}
                <button
                  onClick={() => handleRemoveSub(s)}
                  aria-label={`Remove ${s}`}
                  className="hover:bg-blue-200 rounded-md p-0.5 transition-colors text-blue-700"
                >
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={updateMut.isPending || !subDraft.trim()}
